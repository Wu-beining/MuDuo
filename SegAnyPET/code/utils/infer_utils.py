...
#                 multimask_output=False,  # <--- 显式添加此参数
#             )
...
#     return medsam_seg_mask, medsam_seg_prob, low_res_masks.detach() # Return last low_res_mask as well
...
def sam_model_infer(model,
                    ...
                    num_clicks=1): 
    '''
    Inference for SAM-Med3D using iterative clicking.
    '''
    model.eval()
    # 自动判断 device，避免硬编码
    device = next(model.parameters()).device 
    
    # 1. 快速检查：如果当前 Crop 里根本没有这个器官，直接返回空
    if roi_gt is not None and (roi_gt > 0).sum() == 0:
        # print("Tips: Current ROI has no GT label, skipping...")
        return np.zeros_like(roi_image.cpu().numpy().squeeze(), dtype=np.uint8), \
               np.zeros_like(roi_image.cpu().numpy().squeeze(), dtype=np.float32), \
               None

    with torch.no_grad():
        input_tensor = roi_image.to(device)
        # Image Encoder 必须在这里跑，因为每次 roi_image 都不一样
        image_embeddings = model.image_encoder(input_tensor)
        ...
        # 初始化用于生成 Click 的 Mask
        # 注意：这里初始应该是全 0，表示模型还没预测出任何东西
        # current_pred_mask = torch.zeros_like(roi_gt[0, 0]).to(device) # (D, H, W)
        
        if roi_gt is not None:
            shape_ref = roi_gt[0, 0]
        else:
            # roi_image shape is (1, 1, D, H, W), take (D, H, W)
            shape_ref = roi_image[0, 0]
            
        current_pred_mask = torch.zeros_like(shape_ref).to(device)
        

        # --- 迭代交互循环 ---
        for i in range(num_clicks):
            # 1. 生成新的点击 (基于 GT 和 当前预测的差异)
            if roi_gt is not None:
                new_pt_co, new_pt_la = prompt_generator(
                    current_pred_mask.cpu(), 
                    roi_gt[0, 0].cpu()
                )
                new_pt_co = new_pt_co.to(device)
                new_pt_la = new_pt_la.to(device)
            else:
                # 无 GT 时的备用逻辑 (通常用于测试时的手动点击，这里简化处理)
# >>>>>>> 修改开始 >>>>>>>
                # 无 GT 时的备用逻辑 (通常用于测试时的手动点击，这里简化处理)
                if i == 0: # 第一针打在中心
                    d, h, w = roi_image.shape[2], roi_image.shape[3], roi_image.shape[4]
                    new_pt_co = torch.tensor([[[d // 2, h // 2, w // 2]]], device=device, dtype=torch.float)
                    new_pt_la = torch.tensor([[1]], device=device, dtype=torch.long)
                else:
                    break
            
            # 2. 拼接点击历史
            points_coords = torch.cat([points_coords, new_pt_co], dim=1)
            points_labels = torch.cat([points_labels, new_pt_la], dim=1)

            # 3. Prompt Encoder
            sparse_embeddings, dense_embeddings = model.prompt_encoder(
                points=[points_coords, points_labels],
                boxes=None,
                masks=prev_low_res_mask, # 传入上一轮的 Logit
            )

            # 4. Mask Decoder
            # 关键：multimask_output=False 保证针对当前单类别输出最优结果
            low_res_masks, _ = model.mask_decoder(
                image_embeddings=image_embeddings,
                image_pe=model.prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=sparse_embeddings,
            ...
            # 更新 prev_low_res_mask 用于下一轮
            prev_low_res_mask = low_res_masks

            # 更新 current_pred_mask 用于生成下一次点击
            # 需要插值回原图尺寸，并转为 0/1 Mask
            ...
            current_pred_mask = (torch.sigmoid(pred_high_res) > 0.5).squeeze()
    ...
    medsam_seg_prob = torch.sigmoid(final_masks_hr).cpu().numpy().squeeze()
    medsam_seg_mask = (medsam_seg_prob > 0.5).astype(np.uint8)

    return medsam_seg_mask, medsam_seg_prob, low_res_masks.detach()
...
def data_postprocess(roi_pred_numpy, meta_info):
    ...
    if roi_pred_numpy.ndim == 5:
        roi_pred_numpy = roi_pred_numpy[0, 0] # 去掉 Batch 和 Channel
    elif roi_pred_numpy.ndim == 4:
        roi_pred_numpy = roi_pred_numpy[0] # 去掉多余维度
